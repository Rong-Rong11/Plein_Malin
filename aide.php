...
<main class="page-conteneur page-info">
	<section class="panneau">
		<p class="surtitre"><?= texte_securise("Aide") ?></p>
		...
		<p class="note-discrete"><?= texte_securise("La position vient de l'adresse IP. Elle peut pointer vers une ville proche plutôt que vers l'adresse exacte.") ?></p>

		<h3><?= texte_securise("Pourquoi certains carburants n'apparaissent pas ?") ?></h3>
		<p class="note-discrete"><?= texte_securise("Une station ne propose pas toujours tous les carburants, ou l'API ne fournit pas toujours un prix à jour pour chaque carburant.") ?></p>

		<h3><?= texte_securise("D'où viennent les prix ?") ?></h3>
		<p class="note-discrete"><?= texte_securise("Les prix viennent de l'API officielle des prix des carburants et sont traités côté serveur en PHP.") ?></p>
		</section>
	</section>
</main>
